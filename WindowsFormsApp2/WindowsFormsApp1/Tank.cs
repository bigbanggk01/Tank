...
namespace WindowsFormsApp1
{
    class Tank
    {
        ...
        public void Go_Down(Form f,Map m)
        {
            ...
            SolidBrush sb = new SolidBrush(Color.White);
            Graphics g = f.CreateGraphics();
            if (y[0] == 44 || m.isWall(this.x[0], this.y[0] + 1) == true || m.isWall(this.x[1], this.y[1] + 1) == true || m.isWall(this.x[3], this.y[3] + 1) ){ return; }
            for (int i = 0; i < 6; i++)
            {
                g.FillRectangle(sb, x[i] * 20 + 1, y[i] * 20 + 1, 18, 18);
            }
        ...
        public void Go_Right(Form f,Map m)
        {
            ...
            SolidBrush sb = new SolidBrush(Color.White);
            Graphics g = f.CreateGraphics();
            if(x[0] == 79||m.isWall(this.x[0] + 1, this.y[0]) == true || m.isWall(this.x[1] + 1, this.y[1]) == true || m.isWall(this.x[3] + 1, this.y[3]) == true) { return; }
            for (int i = 0; i < 6; i++)
            {
                g.FillRectangle(sb,x[i] * 20 + 1, y[i] * 20 + 1, 18, 18);
            }
        ...
        public void Go_Left(Form f, Map m)
        {
            ...
            SolidBrush sb = new SolidBrush(Color.White);
            Graphics g = f.CreateGraphics();
            if (x[0] == 0 || m.isWall(x[0] - 1,y[0])==true || m.isWall(x[1] - 1, y[1]) == true || m.isWall(x[3] - 1, y[3]) == true)
            {
                return;
            }
            for (int i = 0; i < 6; i++)
